...
# Class to index an annotation file and to facilitate loading of its contents as annoation objects
class loadTrackFile(object):
    ...
    def __init__(self, fileobj, filetype="", fixedannotype="", indexlines=0, smallonly=0):
        ...
        if filetype=="":
            self.type=libAnnoShared.detectFileType(self.fileobj)
        else:
        ...
        while line:
            if line[0] != "#":                          # Ignore commonly used comment characters
                if self.type=="GTF":
                    ...
                    annoentry = libAnnoShared.Annotation(line,self.type,self.header)
    ...
    def loadItems(self, chr, start, end):
        ...
        if chr not in self.chrIndex:
            ...
            line = self.fileobj.readline()
            while line:
                # TODO *** Candidate for element object use
                annoentry = libAnnoShared.Annotation(line,self.type,self.header)
    ...
    def loadLine(self, line):                       # TODO: Overlaps with above function. Rewrite to reduce the repeated code
